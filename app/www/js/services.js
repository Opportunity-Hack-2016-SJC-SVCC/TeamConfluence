...
var addControler = function(mymap, callback,  options){
	L.Control.Crtl = L.Control.extend(
	{
	    ...
	    onAdd: function (map) {
	        ...
	        if(options.href) controlUI.href = options.href;
	        if(options.placeholder) controlUI.placeholder=options.placeholder;
	        if(options.text) controlUI.value = options.text;
...
/********
START: addMarker : add marker to the pased location.
*********/
var addMark = function(mymap, location, $popup){
	var marker = L.marker([location.lat,location.lng]).addTo(mymap)
		.bindPopup($popup.html()).openPopup();
	return marker;
};
...
// build the poliline form the cord array
var getPoliLine = function(data){
	// var 
	...
	var _Polyline = new L.Polyline(pointList, {
		color: 'red',	
		weight: 3,
		opacity: 0.5,
		smoothFactor: 1
